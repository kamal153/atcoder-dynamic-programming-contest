...
lli flowers(vector<Flower>& v, int n) {
	vlli dp(n + 1);
	map<lli, lli> sortedFlowers;

	dp[1] = v[1].b;
	sortedFlowers[v[1].h] = v[1].b;
	lli ans = dp[1];

	for(int i = 2; i <= n; i++) {
		dp[i] = v[i].b;
		auto it = sortedFlowers.lower_bound(v[i].h + 1);
		if(it != sortedFlowers.begin()) {
			it--;
			dp[i] += it->S;
		}
		sortedFlowers[v[i].h] = dp[i];
		it = sortedFlowers.upper_bound(v[i].h);
		while(it != sortedFlowers.end() && it->S <= dp[i]) {
			auto t = it;
			it++;
			sortedFlowers.erase(t);
		}
		ans = max(ans, dp[i]);
	}
